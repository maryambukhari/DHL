...
<body>
    ...
    <section class="featured">
        <?php
        try {
            $stmt = $pdo->query("SELECT a.*, c.name AS cat_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id WHERE a.title IS NOT NULL AND a.image_url IS NOT NULL AND a.content IS NOT NULL ORDER BY a.publish_date DESC LIMIT 6");
            if ($stmt->rowCount() === 0) {
                echo '<p class="error">No articles available.</p>';
            } else {
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    echo '<div class="article-card">
                        <img src="' . htmlspecialchars($row['image_url']) . '" alt="' . htmlspecialchars($row['title']) . '">
                        <h2>' . htmlspecialchars($row['title']) . '</h2>
                        <p>By ' . htmlspecialchars($row['author']) . ' | ' . htmlspecialchars($row['publish_date']) . ' | ' . htmlspecialchars($row['cat_name'] ?? 'Uncategorized') . '</p>
            ...
            error_log("Error loading articles: " . $e->getMessage(), 3, 'errors.log');
        }
        ?>
    </section>
    <script>
